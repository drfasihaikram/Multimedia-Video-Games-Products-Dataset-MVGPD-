...
#'detection_Classes_Name','title'
result_users['detection_Classes_Name'] = result_users['detection_Classes_Name'].apply(lambda x: ' '.join(x))
result_users['genre'] = result_users[['genre','title', 'genreId','Search_keyword']].apply(lambda x: ' '.join(x), axis=1)
...
result_users.to_json(r'games_action_NoSemantic.json', orient='records', lines=True)
